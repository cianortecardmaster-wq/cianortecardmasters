/* Cianorte Card Masters - bundle público gerado em 2026-06-02.
   Inclui: main.js, card-apis-global.js, spikes.js e community.js.
   Não editar direto sem atualizar também os fontes correspondentes. */
...
})();;(function(){function initCommunityPhotoCarousels(){document.querySelectorAll('.post-style-fotos .gallery-grid').forEach((track,index)=>{if(track.dataset.photoCarouselReady==='true')return;const items=Array.from(track.children).filter((item)=>item.matches('.gallery-item'));if(!items.length)return;track.dataset.photoCarouselReady='true';track.classList.add('photo-carousel-track');track.setAttribute('tabindex','0');track.setAttribute('aria-label',track.getAttribute('aria-label')|| 'Galeria de fotos da comunidade');if(items.length<2)return;const wrapper=document.createElement('div');wrapper.className='photo-carousel-wrap';track.parentNode.insertBefore(wrapper,track);wrapper.appendChild(track);const nav=document.createElement('div');nav.className='photo-carousel-nav';const hint=document.createElement('p');hint.className='photo-carousel-hint';hint.textContent='Arraste a foto para o lado ou use as setas para ver mais.';const actions=document.createElement('div');actions.className='photo-carousel-actions';const prev=document.createElement('button');prev.className='photo-carousel-btn';prev.type='button';prev.innerHTML='‹';prev.setAttribute('aria-label','Foto anterior');const counter=document.createElement('span');counter.className='photo-carousel-counter';counter.setAttribute('aria-live','polite');const next=document.createElement('button');next.className='photo-carousel-btn';next.type='button';next.innerHTML='›';next.setAttribute('aria-label','Próxima foto');actions.append(prev,counter,next);nav.append(hint,actions);wrapper.appendChild(nav);function currentIndex(){const positions=items.map((item)=>Math.abs(item.offsetLeft-track.scrollLeft));return positions.indexOf(Math.min(...positions));}function updateCounter(){counter.textContent=`${currentIndex()+1}/${items.length}`;}function goTo(delta){const targetIndex=Math.max(0,Math.min(items.length-1,currentIndex()+delta));track.scrollTo({left:items[targetIndex].offsetLeft,behavior:'smooth'});}prev.addEventListener('click',()=>goTo(-1));next.addEventListener('click',()=>goTo(1));track.addEventListener('keydown',(event)=>{if(event.key==='ArrowLeft'){event.preventDefault();goTo(-1);}if(event.key==='ArrowRight'){event.preventDefault();goTo(1);}});let scrollTicking=false;track.addEventListener('scroll',()=>{if(scrollTicking)return;scrollTicking=true;window.requestAnimationFrame(()=>{updateCounter();scrollTicking=false;});},{passive:true});let dragging=false;let startX=0;let startLeft=0;let pointerId=null;track.addEventListener('pointerdown',(event)=>{if(event.button !==undefined && event.button !==0)return;dragging=true;pointerId=event.pointerId;startX=event.clientX;startLeft=track.scrollLeft;track.classList.add('is-dragging');track.setPointerCapture(pointerId);});track.addEventListener('pointermove',(event)=>{if(!dragging)return;event.preventDefault();track.scrollLeft=startLeft-(event.clientX-startX);});function stopDragging(){if(!dragging)return;dragging=false;track.classList.remove('is-dragging');if(pointerId !==null && track.hasPointerCapture(pointerId)){track.releasePointerCapture(pointerId);}pointerId=null;}track.addEventListener('pointerup',stopDragging);track.addEventListener('pointercancel',stopDragging);track.addEventListener('pointerleave',stopDragging);updateCounter();window.addEventListener('resize',updateCounter);});}document.addEventListener('DOMContentLoaded',initCommunityPhotoCarousels);})();
